...
public interface LessonRepository extends JpaRepository<Lesson, UUID> {
    ...
    // ───── PUBLISHED only (student/public view) ─────
    @Query("""
           ...
           """)
    List<Lesson> findPublishedByModuleId(@Param("moduleId") UUID moduleId);
    ...
    @Query("""
           ...
           ORDER BY l.displayOrder ASC
           """)
    List<Lesson> findPublishedByCourseId(@Param("courseId") UUID courseId);
    ...
    @Query("""
           SELECT MAX(l.displayOrder) FROM Lesson l
           WHERE l.module.id = :moduleId AND l.isDeleted = false
           """)
